"""
Converts the official list of functions, found in https://support.google.com/docs/table/25273?Hl=en, into functions the program can use
"""


data = """
AI	AI	AI() or Gemini()	Generates text, summarizes information, categorizes information, and accesses real-time information. This function is a Google Workspace with Gemini feature
...
Array	CHOOSECOLS	CHOOSECOLS(array, col_num1, [col_num2]) 	Creates a new array from the selected columns in the existing range. Learn more
Array	CHOOSEROWS	CHOOSEROWS(array, row_num1, [row_num2])	Creates a new array from the selected rows in the existing range. Learn more
...
Array	HSTACK	HSTACK(range1; [range2, …])	Appends ranges horizontally and in sequence to return a larger array. Learn more
...
Array	TOCOL	TOCOL(array_or_range, [ignore], [scan_by_column])	Transforms an array or range of cells into a single column. Learn more
Array	TOROW	TOROW(array_or_range, [ignore], [scan_by_column])	Transforms an array or range of cells into a single row. Learn more
...
Array	VSTACK	VSTACK(range1; [range2, …])	Appends ranges vertically and in sequence to return a larger array. Learn more
Array	WRAPCOLS	WRAPCOLS(range, wrap_count, [pad_with])	Wraps the provided row or column of cells by columns after a specified number of elements to form a new array. Learn more
Array	WRAPROWS	WRAPROWS(range, wrap_count, [pad_with])	Wraps the provided row or column of cells by rows after a specified number of elements to form a new array. Learn more
...
Date	EDATE	EDATE(start_date, months)	Returns a date a specified number of months before or after another date. Learn more
...
Date	EPOCHTODATE	EPOCHTODATE(timestamp, [unit]) 	Converts a Unix epoch timestamp in seconds, milliseconds, or microseconds to a datetime in UTC. Learn more
...
Google	ARRAYFORMULA	ARRAYFORMULA(array_formula)	Enables the display of values returned from an array formula into multiple rows and/or columns and the use of non-array functions with arrays. Learn more
...
Google	QUERY	QUERY(data, query, [headers])	Runs a Google Visualization API Query Language query across data. Learn more
...
Info	SHEETS	SHEETS(reference)	Returns the total number of sheets in the referenced spreadsheet. Learn more about the SHEETS function.
...
Lookup	GETPIVOTDATA	GETPIVOTDATA(value_name, any_pivot_table_cell, [original_column, ...], [pivot_item, ...]	Extracts an aggregated value from a pivot table that corresponds to the specified row and column headings. Learn more
...
Lookup	INDIRECT	INDIRECT(cell_reference_as_string, [is_A1_notation])	Returns a cell reference specified by a string. Learn more
...
Lookup	SHEET	SHEET(value)	Returns the sheet number of the specified sheet or other reference. Learn more about the SHEET function.
...
Statistical	MARGINOFERROR	MARGINOFERROR(range, confidence)	Calculates the amount of random sampling error given a range of values and a confidence level. Learn more
...
Text	MIDB	MIDB(string, starting_at, extract_length_bytes)\tReturns a section of a string starting at a given character and up to a specified number of bytes. Learn more.
...
Text	SPLIT	SPLIT(text, delimiter, [split_by_each], [remove_empty_text])	Divides text around a specified character or string, and puts each fragment into a separate cell in the row. Learn more
...
Web	HYPERLINK	HYPERLINK(url, [link_label])	Creates a hyperlink inside a cell. Learn more
Web	IMPORTDATA	IMPORTDATA(url)	Imports data at a given url in .csv (comma-separated value) or .tsv (tab-separated value) format. Learn more
Web	IMPORTFEED	IMPORTFEED(url, [query], [headers], [num_items])	Imports a RSS or ATOM feed. Learn more
Web	IMPORTHTML	IMPORTHTML(url, query, index)	Imports data from a table or list within an HTML page. Learn more
Web	IMPORTRANGE	IMPORTRANGE(spreadsheet_url, range_string)	Imports a range of cells from a specified spreadsheet. Learn more
Web	IMPORTXML	IMPORTXML(url, xpath_query)	Imports data from any of various structured data types including XML, HTML, CSV, TSV, and RSS and ATOM XML feeds. Learn more
Web	ISURL	ISURL(value)	Checks whether a value is a valid URL. Learn more
"""
from typing import Optional
Optional[dict[str, str]]

def detect_param_type(param: str, funcname: str, functype: str) -> str:
    match param.lower():
        case 'string': return 'str'
        case 'rows' | 'column' | 'n': return 'int'
        case 'time': return 'Time'
        case 'complex_number': return 'ComplexNumber'
        case 'cumulative': return 'bool'

    if 'text' in param:
        return 'str'
        
    if param == 'unit' and funcname in {'DATEDIF', 'EPOCHTODATE'}:
        return 'DateUnit'

    if 'language' in param:
        return 'LanguageCodes'

    if 'ROUND' in funcname and param == 'value':
        return 'float'

    if param in {'probability', 'confidence', 'percentile', 'standard_deviation'}:
        return "PercentFloat"

    if 'degrees' in param or param in {'x', 'y', 'mean', 'value1', 'value2', 'imaginary_part'}:
        return 'float'

    if functype == 'MATH' and param.startswith('value'):
        return 'float'

    if 'num' in param or 'length' in param or param in {'significant_digits', 'shift_amount', 'months', 'year', 'day', 'week', 'month'} or 'count' in param: return 'int'
    if 'range' in param or 'data_' in param: return 'Range'
    if 'array' in param: return 'Array'
    if 'date' in param: return 'Date'
    if 'url' in param: return 'URL'
...
for line in data.splitlines():
    try:
        type, name, fullfunction, desc = (line.rstrip().split('\t'))

        # Replace cell
        if name == 'CELL':
            name = 'CELLINFO'

        desc = desc.rstrip('.').rstrip(' Learn more')
        name = name.replace('.', '_')
    except ValueError: 
        continue

    # Get parameters
    params = fullfunction.split('(',1)[1].split(')')[0].replace(' ', '').replace('…', '.').replace(';', ',')+', '
    ...
    for i, t in enumerate(params):
        if t == '[': optional = True
            ...
            ty = []
            for temp2 in temp.replace("_or_", '|').split('|'):
                ty.append(detect_param_type(temp2, name, type.upper()))
            ty = ' | '.join(set(ty))
            if ty != "Any":
                ty += f" | Cell[{ty}]"

            # Replace certain keywords
            temp = temp.replace('yield', 'yield_value').replace('|', '_OR_').replace('_or_', '_OR_')

            if params[i+1] == '.' and not alreadyAll:
                result += '*, '
                alreadyAll = True
            ...
            if optional:
                ty = f"Optional[{ty}]"
            if temp != '':
                result += temp + (': '+ty if ty is not None else '') + ', '
            temp = ''
        else:
            temp += t

    if ('whether' in desc.lower() and name not in {"T_TEST", "FTEST"}) or ("true" in desc.lower() and "false" in desc.lower()):
        returnType = 'bool'        

    elif (name in {'DATEDIF'}) or ("integer" in desc) or (type.upper() == "TEXT" and (name.rstrip('B') in {"SEARCH", "FIND", "LEN", "CODE", "UNICHAR", "UNICODE"})):
        returnType = 'int'

    elif (name in {'TIME', 'NOW', 'EPOCHTODATE', 'EDATE', 'EOMONTH', 'DATEVALUE', 'DATE'}) or "date value" in desc.lower():
        returnType = 'Date'

    elif name in {"HYPERLINK", "ENCODEURL"}:
        returnType = 'URL'

    elif name == 'COMPLEX':
        returnType = 'ComplexNumber'

    elif (name in {'VALUE'}) or (desc.startswith("Returns") and "number" in desc.lower() and name not in {'MIDB'}) or ("ROUND" in name) or (name.startswith("NORM")):
        returnType = 'float'

    elif (type.upper() == 'MATH' and 'range' not in desc and 'array' not in desc):
        returnType = 'float'

    elif (type.upper() == "TEXT"):
        returnType = 'str'

    result = result.rstrip(', ') + ")" + (f" -> {returnType}" if returnType else '') + ":\n    '''" + desc.strip(' ') + "'''"
